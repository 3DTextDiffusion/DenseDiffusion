...
def process_sketch(canvas_data):
    ...
    for color in colors:
        ...
        if any(c != 255 for c in (r, g, b)):
            binary_matrix = create_binary_matrix(im2arr, (r, g, b))
            binary_matrixes.append(binary_matrix)
            binary_matrix_ = np.repeat(
                np.expand_dims(binary_matrix, axis=(-1)), 3, axis=(-1)
            )
            colored_map = binary_matrix_ * (r, g, b) + (1 - binary_matrix_) * (
                50,
                50,
                50,
            )
...
def process_prompts(binary_matrixes, *seg_prompts):
    return [
        ...
        gr.update(value=" , ".join(seg_prompts[: len(binary_matrixes)])),
    ]
