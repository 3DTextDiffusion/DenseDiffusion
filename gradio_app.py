...
def process_generation(
    binary_matrixes, seed, creg_, sreg_, sizereg_, bsz, master_prompt, *prompts
):
    ...
    layouts = torch.cat(
        [preprocess_mask(mask_, sp_sz, sp_sz, device) for mask_ in binary_matrixes]
    )
    ...
    for r in range(4):
        ...
        reg_sizes[np.power(res, 2)] = 1 - sizereg * layouts_s.sum(-1, keepdim=True) / (
            np.power(res, 2)
        )
    ...
    for i in range(1, len(prompts)):
        ...
        for j in range(77):
            try:
                ...
                raise gr.Error(
                    "Please check whether every segment prompt is included in the full text !"
                )
                return
    ...
    if seed == -1:
        ...
        latents = torch.randn(
            bsz, 4, sp_sz, sp_sz, generator=torch.Generator().manual_seed(seed)
        ).to(device)
...
with gr.Blocks(css=css) as demo:
        ...
        with gr.Box(elem_id="main-image"):
                ...
                for n in range(MAX_COLORS):
                    if n == 0:
                        with gr.Row(visible=False) as color_row[n]:
                            ...
                            prompts.append(
                                gr.Textbox(
                                    label="Prompt for the background (white region)",
                                    value="",
                                )
                            )
                    else:
                        with gr.Row(visible=False) as color_row[n]:
                            ...
                            prompts.append(
                                gr.Textbox(label="Prompt for the segment " + str(n))
                            )

                get_genprompt_run = gr.Button(
                    "(2) I've finished segment labeling ! 😺",
                    elem_id="prompt_button",
                    interactive=True,
                )
            ...
            with gr.Column(visible=False) as gen_prompt_vis:
                general_prompt = gr.Textbox(
                    value="",
                    label="(3) Textual Description for the entire image",
                    interactive=True,
                )
                with gr.Accordion("(4) Tune the hyperparameters", open=False):
                    creg_ = gr.Slider(
                        label=" w\u1d9c (The degree of attention modulation at cross-attention layers) ",
                        minimum=0,
                        maximum=2.0,
                        value=1.0,
                        step=0.1,
                    ...
                    sreg_ = gr.Slider(
                        label=" w \u02e2 (The degree of attention modulation at self-attention layers) ",
                        minimum=0,
                        maximum=2.0,
                        value=0.3,
                        step=0.1,
                    ...
                    sizereg_ = gr.Slider(
                        label="The degree of mask-area adaptive adjustment",
                        minimum=0,
                        maximum=1.0,
                        value=1.0,
                        step=0.1,
                    ...
                    seed_ = gr.Slider(
                        label="Seed", minimum=-1, maximum=999999999, value=-1, step=1
                    )
    ...
    button_run.click(
        process_sketch,
        inputs=[canvas_data],
        outputs=[post_sketch, binary_matrixes, *color_row, *colors],
        _js=get_js_colors,
        queue=False,
    )

    get_genprompt_run.click(
        process_prompts,
        inputs=[binary_matrixes, *prompts],
        outputs=[gen_prompt_vis, general_prompt],
        queue=False,
    )
    ...
    gr.Examples(
        examples=[
            [
                val_layout + "0.png",
                "***".join(
                    [val_prompt[0]["textual_condition"]]
                    + val_prompt[0]["segment_descriptions"]
                ),
                381940206,
            ],
            [
                val_layout + "1.png",
                "***".join(
                    [val_prompt[1]["textual_condition"]]
                    + val_prompt[1]["segment_descriptions"]
                ),
                307504592,
            ],
            [
                val_layout + "5.png",
                "***".join(
                    [val_prompt[5]["textual_condition"]]
                    + val_prompt[5]["segment_descriptions"]
                ),
                114972190,
            ],
        ],
